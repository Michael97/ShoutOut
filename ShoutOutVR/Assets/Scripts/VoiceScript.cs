...
{
    public AudioMixerGroup _audioOutput;         /// This is our silent mixer to prevent audio playback.

    private const int _sampleFrequency = 44100;  /// Common sampling rate for recording analog audio.
    ...
    void Awake()
    {
        ...
        GetComponent<AudioSource>().clip = Microphone.Start(null, false, 3600, _sampleFrequency);

        // Set the audio mixer to our custom made silent mixer (this prevents audio playback).
        GetComponent<AudioSource>().outputAudioMixerGroup = _audioOutput;
