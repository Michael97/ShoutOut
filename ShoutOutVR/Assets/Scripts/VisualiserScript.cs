...
{
    public delegate void VisualiserScriptDelegate(int barPos);
    public static event VisualiserScriptDelegate PitchThresholdReached;
    ...
    void Start()
    {
        // Set the initial position and colours of the bars.
        SetBarColours();
    ...
    {
        // Smoothly rotates the bars so that they face the player along the horizontal plane.
        transform.forward = Vector3.Lerp(transform.forward, Vector3.Cross(Camera.main.transform.right, Vector3.up), .1f);
    }
    ...
    private void SetBarSize()
    {
        ...
        for (int i = 0; i < _audioVisualiserBars.Length; i++)
        {
            ...
            if (lerpZ >= _screenHeight)
            {
                ...
                if (PitchThresholdReached != null)
                {
                    PitchThresholdReached.Invoke(i);
                }
            }
